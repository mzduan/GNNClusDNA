...
def graph_propagation(edges, score):
    th=score.min()
    edges = np.sort(edges, axis=1)  # edges[0] > edges[1]
    nodes = np.sort(np.unique(edges.flatten()))
    mapping = -1 * np.ones((nodes.max()+1), dtype=np.int)
    mapping[nodes] = np.arange(nodes.shape[0])
    link_idx = mapping[edges]

    #把杂乱无章的节点编号 map 到 0 ~ nodes.shape[0]之间
    ...
    comps= connected_components_constraint(vertexs,score_dict,0.66)
    components = comps[:]
    # while remain:
    #     th = th + (1 - th) * step
    #     iter=iter+1
...
if __name__ == '__main__':
    ...
    ckpt = load_checkpoint("/Users/duan/Desktop/ClustRead/data/S7/model.ckpt")
    net = gcn()
    net.load_state_dict(ckpt)
    net = net.eval()
    testset = Test_Feeder("/Users/duan/Desktop/ClustRead/data/S7/features.npy",
                      "/Users/duan/Desktop/ClustRead/data/S7/KNN.npy",
                      "/Users/duan/Desktop/ClustRead/data/S7/labels.npy",
                     )
    ...
    components=graph_propagation(edges,scores)
    # components=bfs(edges,scores)

    #part 3:

    # 得到groundtruth
    true_labels=np.load("/Users/duan/Desktop/ClustRead/data/S7/labels.npy")
